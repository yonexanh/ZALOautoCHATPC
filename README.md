...
Nếu dùng bản `.app`/`.dmg` đã đóng gói, máy sử dụng không cần cài `Python 3`, `Swift` hay `Xcode`. Các công cụ đó chỉ cần trên máy build/phát triển.
...
## Đóng gói chuyển máy mới

```bash
python3 /Users/mac/Documents/ZALOautoCHATPC/package_release.py
```

Lệnh này tạo gói universal cho cả Apple Silicon và Intel:

- `release/ZaloSchedulerLauncher-<version>-<build>-macOS-universal.dmg`
- `release/ZaloSchedulerLauncher-<version>-<build>-macOS-universal.zip`

Trên máy mới:

- Kéo `ZaloSchedulerLauncher.app` vào `Applications`
- Mở Zalo và đăng nhập tài khoản cần gửi
- Mở app, cấp `Accessibility`, chọn đúng bản Zalo đang mở
- Kiểm tra lại đường dẫn ảnh/video nếu chuyển cấu hình từ máy cũ
...
- scheduler trong launcher chạy native, không gọi `python3 main.py run`
